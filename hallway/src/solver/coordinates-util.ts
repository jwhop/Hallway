...
export default class CoordinatesUtil {
  ...
  static convert(
    ...
    imageWidth: number,
    imageHeight: number): THREE.Vector2 {
  ...
  private static absoluteToRelative(point: THREE.Vector2, imageWidth: number, imageHeight: number): THREE.Vector2 {
  ...
  private static relativeToAbsolute(point: THREE.Vector2, imageWidth: number, imageHeight: number): THREE.Vector2 {
    return new THREE.Vector2(
      point.x * imageWidth,
      point.y * imageHeight
    )
  ...
  private static relativeToImagePlane(point: THREE.Vector2, imageWidth: number, imageHeight: number): THREE.Vector2 {
    let aspectRatio = imageWidth / imageHeight
    if (aspectRatio <= 1) {
      // tall image. [0, 1] x [0, 1] => [-aspect, aspect] x [-1, 1]
      return new THREE.Vector2(
        (-1 + 2 * point.x) * aspectRatio,
        1 - 2 * point.y
      )
    } else {
      ...
      return new THREE.Vector2(
        -1 + 2 * point.x,
        (1 - 2 * point.y) / aspectRatio
      )
    }
  }

  private static imagePlaneToRelative(point: THREE.Vector2, imageWidth: number, imageHeight: number): THREE.Vector2 {
    let imageAspect = imageWidth / imageHeight
    if (imageAspect <= 1) {
      // tall image. [-aspect, aspect] x [-1, 1] => [0, 1] x [0, 1]
      return new THREE.Vector2(
        0.5 * (point.x / imageAspect + 1),
        0.5 * (-point.y + 1)
      )
    } else {
